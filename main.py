...
def scatterPlotDrawer2(x,y):
    fig = plt.figure(figsize=(20,15))
    sns.set_style("whitegrid")
    sns.scatterplot(data=df2, x=x, y=y,  palette="deep", sizes=(20, 200), legend="full")
    plt.xlabel(x, fontsize=22)
    plt.ylabel(y, fontsize=22)
    plt.xticks(rotation=90, fontsize=18)
    plt.legend(prop={'size': 18})
...
def main():
    ...
    img = get_img_as_base64_with_transparency("Green Blue Motivational Quote Landscape Desktop Wallpaper.PNG", transparency)
    ...
    if select_type == 'CROP PREDICTION':
        ...
        if select_type == 'HOME':
           st.title("On the occasion of National Science Day, I am delighted to present...")
...
ALPINE is a data-driven project dedicated to promoting sustainable farming in Uttarakhand, India. Utilizing Python libraries such as NumPy, Pandas, Matplotlib, and Streamlit. ALPINE empowers farmers to make informed decisions regarding crop management, resource allocation, and environmental impact.
    ...
    """<p style="font-size:20px;">
        ...
        if select_type == "Predict Your soil":
            ...
            st.header("To predict your soil, give values")
            st.subheader("Drag to Give Values")
            ...
            EC = st.slider('Electrical Conductivity (EC)', min_value=min_EC, max_value=max_EC, value=min_EC, step=0.01)
            OC = st.slider('Organic Carbon (OC)', min_value=min_OC, max_value=max_OC, value=min_OC, step=0.01)
            S = st.slider('Sulfur (S)', min_value=min_S, max_value=max_S, value=min_S, step=1.0)
            Zn = st.slider('Zinc (Zn)', min_value=min_Zn, max_value=max_Zn, value=min_Zn, step=1.0)
            Fe = st.slider('Iron (Fe)', min_value=min_Fe, max_value=max_Fe, value=min_Fe, step=1.0)
            Cu = st.slider('Copper (Cu)', min_value=min_Cu, max_value=max_Cu, value=min_Cu, step=0.01)
            Mn = st.slider('Manganese (Mn)', min_value=min_Mn, max_value=max_Mn, value=min_Mn, step=1.0)
            B = st.slider('Boron (B)', min_value=min_B, max_value=max_B, value=min_B, step=0.01)

            if st.button("Predict your soil"):
                output = predict_soil(N, P, K, pH, EC, OC, S, Zn, Fe, Cu, Mn, B)
                output_str = str(output).capitalize()
                if output_str == 0:
                    st.success(f'This soil is not Good for cultivate')
                else:
                    st.success(f'This soil is Good for cultivate')
